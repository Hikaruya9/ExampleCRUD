...
//    private static String CREATE_CLIENT = "INSERT INTO client(name,cpfCnpj,email,phoneNumber,adress) VALUES(?,?,?,?,?)";
    ...
    private static String UPDATE_CLIENT = "UPDATE client SET name = ?,cpf_cnpj = ?,email = ?,phone_number = ?,adress = ? WHERE id = ?";
    private static String DELETE_CLIENT = "DELETE FROM client WHERE id = ?";
    private static String LIST_CLIENTS = "SELECT id,name,cpf_cnpj,email,phone_number,adress FROM client";
    private static String RETRIEVE_USER = "SELECT id,username,password FROM user WHERE username = ? AND passowrd = ?";
    
    public void createClient(Client client){
        String sql = "INSERT INTO client(name,cpf_cnpj,email,phone_number,adress) VALUES(?,?,?,?,?)";
    ...
    public Client retrieveClient(String id) throws Exception{
        ...
        try{
            ...
            rs = pst.executeQuery();
            if(rs.next()){
                client = new Client(rs.getString("id"), rs.getString("name"), rs.getString("cpf_cnpj"), rs.getString("email"), rs.getString("phone_number"), rs.getString("adress"));
            }
        }catch(SQLException e){
            e.printStackTrace();
        }
    ...
    public ArrayList<Client> listClients() throws Exception{
        ...
        try{
            ...
            rs = pst.executeQuery();
            while(rs.next()){
                clients.add(new Client(rs.getString("id"), rs.getString("name"), rs.getString("cpf_cnpj"), rs.getString("email"), rs.getString("phone_number"), rs.getString("adress")));
            }
        }catch(SQLException e){
            e.printStackTrace();
        }
